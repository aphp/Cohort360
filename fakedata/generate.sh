...
curl -XPOST -H 'Content-type: application/json' http://fhir:8080/fhir -d 
...
update_status 15 "Loading patients data" "16 minutes" 6
...
# Update status every 10 files to avoid too many updates
  if (( current_file % 10 == 0 )); then
    ...
    update_status $subtask_percent "Loading patients data ($current_file/$total_files)" "$estimated_time" 4
  fi
