 ...
 * @param mainDiagnosisList : the main diagnosis list
 * @returns : the last three diagnosis labels
 */
...
>(
  ...
  const newSelectedItems = selectedItems.filter((item, index, array) => {
    ...
    const parentItem: any = returnParentElement(rootRows, undefined)

    if (parentItem !== undefined) {
      const selectedChildren =
        parentItem.subItems && parentItem.subItems.length > 0
          ? parentItem.subItems.filter((subItem: { id: string }) => !!array.find(({ id }) => id === subItem.id))
          : []
...
const expandRequest = async (
  ...
  } else if (resourceType.toLowerCase() === CriteriaType.OBSERVATION.toLowerCase()) {
  ...
  } else if (resourceType.toLowerCase() === CriteriaType.CLAIM.toLowerCase()) {
    type = 'claim'
  } else if (resourceType.toLowerCase() === CriteriaType.PROCEDURE.toLowerCase()) {
    type = 'procedure'
  } else if (resourceType.toLowerCase() === CriteriaType.CONDITION.toLowerCase()) {
    type = 'condition'
  } else {
    return undefined
  }
...
const expandSingleResourceItem = async (
  ...
  const expandItemAndSubItems = async (
    ...
    if (!selectedCodes.find((selectedItem) => selectedItem.id === itemToExpand.id)) {
      ...
      const subItems = itemToExpand.subItems?.filter((item: Hierarchy<any, any>) => parentsList.find((code) => code.id === item.id)) ?? []
  ...
  const getHigherParentFromList = (parentsList: Hierarchy<any, any>[]): Hierarchy<any, any> | undefined => {
    const higherParentCode: Hierarchy<any, any> | undefined =
