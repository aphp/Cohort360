...
import { scopeLevelsToRequestParam } from 'mappers/perimeters'
...
const servicesPerimeters: IServicePerimeters = {
  ...
  fetchPopulationForRequeteur: async (cohortIds) => {
    ...
    if (ids) {
      const response = await (
        await servicesPerimeters.getRights({ limit: -1, cohortIds: ids, sourceType: SourceType.ALL })
      ).results
      if (!response.length)
        population = [
          {
  ...
  getRights: async (options?: FetchScopeOptions, signal?: AbortSignal): Promise<Back_API_Response<ScopeElement>> => {
    ...
    try {
      ...
      if (options) {
        ...
        if (options.sourceType) params.push(`type_source_value=${scopeLevelsToRequestParam(options.sourceType)}`)
  ...
  getPerimeters: async (
    ...
    const response: Back_API_Response<ScopeElement> = { results: [], count: 0 }
    try {
      let baseUrl: string = 'accesses/perimeters/'
      const params = []
      if (options) {
        if (options.sourceType) params.push(`type_source_value=${scopeLevelsToRequestParam(options.sourceType)}`)
        if (options.limit) params.push(`limit=${options.limit}`)
