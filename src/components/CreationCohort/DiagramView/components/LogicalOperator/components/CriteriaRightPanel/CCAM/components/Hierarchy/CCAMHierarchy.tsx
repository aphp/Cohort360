...
type ProcedureHierarchyProps = {
  ...
  goBack: () => void
  onChangeSelectedHierarchy: (data: Hierarchy<any, any>[] | null | undefined, newHierarchy?: Hierarchy<any, any>[]) => void
  isEdition?: boolean
  onConfirm: () => void
}
...
const ProcedureHierarchy: React.FC<ProcedureHierarchyProps> = (props) => {
  ...
  useEffect(() => {
    ...
    newList.code?.map((item: Hierarchy<any, any>) => findEquivalentRowInItemAndSubItems(item, ccamHierarchy).equivalentRow)
    setCurrentState(newList)
  }, [initialState, ccamHierarchy])

  const _handleClick = (newSelectedItems: Hierarchy<any, any>[] | null | undefined, newHierarchy?: Hierarchy<any, any>[]) => {
